...
import { VoluntarilyCancellationCertificateGenerator } from "../util/document-generators/voluntarilyCancellationCertificate.gen";
...
export class CreditRetirementSlService {
  constructor(
    ...
    private retirementRepo: Repository<CreditRetirementSl>,
  ...
  async createCreditRetirementRequest(
    ...
    if (user.companyId != creditRetirementDto.fromCompanyId) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.cannotRetireForOthers",
          []
        ),
    ...
    if (
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.toCompanyIdRequired",
          []
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    if (!programme) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.programmeNotExist",
          [creditRetirementDto.programmeId]
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    if (
      programme.purposeOfCreditDevelopment !== creditRetirementDto.creditType
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.creditTypeMismatch",
          []
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    if (
      ...
      programme.creditBalance < creditRetirementDto.creditAmount
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.insufficientCredit",
          []
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    if (!fromCompany) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.companyDoesNotExist",
          [creditRetirementDto.fromCompanyId]
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    if (creditRetirementDto.toCompanyId) {
      toCompany = await this.companyService.findByCompanyId(
        creditRetirementDto.toCompanyId
      );
    ...
    const previousRef = await this.findLatestTxRefByProgrammeId(
      creditRetirementDto.programmeId
    );
  ...
  async queryRetirements(
    ...
    if (resp && resp.length > 0) {
      for (const e of resp[0]) {
        e.toCompany =
          e.toCompany.length > 0 && e.toCompany[0] === null ? [] : e.toCompany;
      }
    }
  ...
  async updateCreditRetirementRequestStatus(
    dto: CreditRetirementStatusUpdateSlDto,
    user: User
  ) {
    ...
    if (!retirementRequest) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.requestNotExist",
          [dto.requestId]
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    if (retirementRequest.status !== RetirementStatusSl.PENDING) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "creditRetirementSl.notPendingRequest",
          [dto.requestId]
        ),
        HttpStatus.BAD_REQUEST
      );
    ...
    switch (dto.status) {
      ...
      default:
        throw new HttpException(
          this.helperService.formatReqMessagesString(
            "creditRetirementSl.incorrectStatus",
            [dto.status]
          ),
          HttpStatus.BAD_REQUEST
        );
  ...
  async approveCreditRetirementRequest(
    ...
    try {
      ...
      this.logger.error(
        "Approve Credit Retirement Request Failed due to {}",
        error
      );
      throw new HttpException(
        this.helperService.formatReqMessagesString(
