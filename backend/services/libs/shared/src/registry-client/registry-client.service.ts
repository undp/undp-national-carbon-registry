...
export class RegistryClientService {
  ...
  public async CompanyUpdate(
    organisationSyncRequestDto: OrganisationSyncRequestDto
  ) {
    const response = await this.httpUtilService.sendHttpPut(
      "/national/organisation/sync",
  ...
  public async authProgramme(actionProps: any) {
    if (this.configService.get("systemType") == SYSTEM_TYPE.CARBON_UNIFIED) {
      ...
      const companyIds: FindOrganisationQueryDto = {
        companyIds: [Number(actionProps.authOrganisationId)],
      };
      const authOrganisationName = (
        await this.companyService.findByCompanyIds(companyIds)
      )[0].name;
      ...
      programme.companyId.forEach(async (companyId) => {
        await this.emailHelperService.sendEmailToOrganisationAdmins(
          ...
          EmailTemplates.PROGRAMME_AUTHORISATION,
          {
            ...
            programmePageLink:
              hostAddress +
              `/programmeManagement/view/${actionProps.programmeId}`,
          },
          undefined,
          undefined,
          undefined,
      ...
      const resp = await this.httpUtilService.sendHttpPut(
        "/national/programme/authProgramme",
        actionProps
      );
      return resp;
    }
  ...
  public async rejectProgramme(reject) {
    const resp = await this.httpUtilService.sendHttpPut(
      "/national/programme/rejectProgramme",
      reject
    );
    return resp;
  }
  ...
  public async issueCredit(issue) {
    const resp = await this.httpUtilService.sendHttpPut(
      "/national/programme/issueCredit",
      issue
    );
    return resp;
  }
  ...
  public async programmeAccept(document: any) {
    const resp = await this.httpUtilService.sendHttp(
      "/national/programme/acceptProgramme",
      document
    );
    return resp;
  }
  ...
  public async addDocument(document: ProgrammeDocumentDto) {
    const resp = await this.httpUtilService.sendHttp(
      "/national/programme/addDocument",
      document
    );
    return resp;
  }
  ...
  public async createProgramme(programme: ProgrammeDto) {
    ...
    if (
      programme.ndcAction &&
      (programme.ndcAction.action === NDCActionType.Mitigation ||
        programme.ndcAction.action === NDCActionType.CrossCutting) &&
      programme.ndcAction.typeOfMitigation
    ) {
      programmeReq["mitigationActions"] = [
        this.createNDCReq(programme.ndcAction),
      ];
    }
    const resp = await this.httpUtilService.sendHttp(
  ...
  public async updateOwnership(req: any) {
    const resp = await this.httpUtilService.sendHttp(
      "/national/programme/updateOwnership",
      req
    );
    return resp;
  }
  ...
  public async addMitigation(ndc: NDCAction) {
    ...
    const resp = await this.httpUtilService.sendHttp(
      "/national/programme/addMitigation",
      {
  ...
  public async addNationalInvestment(investment: InvestmentSyncDto) {
    const resp = await this.httpUtilService.sendHttp(
      "/national/organisation/addInvestment",
      investment
    );
    return resp;
  }
  ...
  private createNDCReq(ndc: NDCAction | NDCActionDto) {
    ...
    return {
      ...
      systemEstimatedCredits: ndc.ndcFinancing?.systemEstimatedCredits
        ? ndc.ndcFinancing?.systemEstimatedCredits
        : 0,
      actionId: ndc.id,
