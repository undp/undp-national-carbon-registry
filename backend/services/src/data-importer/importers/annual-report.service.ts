...
export class AnnualReportImport implements ImporterInterface {
  ...
  async start(type: string): Promise<any> {
    ...
    const annualreporturl = await this.annualReportGen.generateAnnualReportpdf(
      year
    );
    const d = await this.documentRepo.query(
      `SELECT "programmeId" FROM public.programme_document WHERE "programmeId"='AR${year}'`
    );
