  ...
  async scrape() {
    try {
      ...
      const activeRowElements = $(
        "div:nth-child(2) table:nth-child(3) tbody:nth-child(1) tr"
      );
      for (let index = 0; index < activeRowElements.length; index++) {
        ...
        if (typeof refNumber == "string" && refNumber.trim().length != 0) {
          const url =
            "https://cdm.unfccc.int/DOE/list/DOE.html?entityCode=" + refNumber;
          await axios.get(url).then((response) => {
            const certdata = response.data;
            const $ = cheerio.load(certdata);
            const numberTable = $(
              "table.formTable:nth-child(1) tbody:nth-child(1) tr:nth-child(5)"
            );
            let number: string;
            numberTable.each(function () {
              let numberspan = $(this).find("td:nth-child(2)").text().trim();
              const numbermatch = String(
                numberspan.match(/Tel:\s*([+0-9\s-]+)/g)
              );
              ...
              if (number == "null") {
                const mobnumbermatch = String(
                  numberspan.match(/Mobile:\s*([+0-9\s-]+)/g)
                );
                const mobnumbers = mobnumbermatch
                  .replace(/^Mobile:\s*/, "")
                  .trim();
            ...
            const alternativeAddress = $(
              "table.formTable:nth-child(1) tbody:nth-child(1) tr:nth-child(4)"
            );
            let address: string;
            addressTable.each(function () {
              let addressspan = $(this)
                .find("tbody:nth-child(1)")
                .text()
                .trim()
                .replace(/\n/g, " ");
              const addrsreplace =
                /Address:([\s\S]*?)(?=Postal code:|\n\n|$)/.exec(addressspan);
              const postcodereplace =
                /Postal code:([\s\S]*?)(?=City:|\n\n|$)/.exec(addressspan);
              const cityreplace = /City:([\s\S]*?)(?=Country:|\n\n|$)/.exec(
                addressspan
              );
              const countrymatch = String(
                addressspan.match(/Country:\s*([\s\S]*)/g)
              );
              const countryreplace = countrymatch
                .replace(/^Country:\s*/, "")
                .trim();
              address =
                addrsreplace[1].trim() +
            ...
            if (!address) {
              alternativeAddress.each(function () {
                let alternativeAddresssnap = $(this)
                  .find("td:nth-child(2)")
                  .text()
                  .trim();
                const removespaces = String(alternativeAddresssnap.split("\n"));
                const alternativeAddressmatch = String(
                  removespaces.match(
                    /^(.*?)(?= Declaration of other offices \(CDM-DOO-FORM\))/g
                  )
                );
      ...
      const activeRowsfinal = [
        ...new Map(
          activeRows.map((item) => [item["refNumber"], item])
        ).values(),
      ];
      // const ObjectsToCsv = require('objects-to-csv');
      // new ObjectsToCsv(activeRowsfinal).toDisk('./active_users.csv', { append: true });

      $("table.formTable:nth-child(7) tbody:nth-child(1) tr").each(
        (index, element) => {
          const refNumber = $(element).find("td:nth-child(1)").text().trim();
          const entity = $(element).find("td:nth-child(2) a").text().trim();
          const oraganization = entity.replace(/\([^)]*\)/g, "").trim();
      ...
      const deactiveRowsfinal = [
        ...new Map(
          deactiveRows.map((item) => [item["refNumber"], item])
        ).values(),
      ];
  ...
  async start(): Promise<any> {
    ...
    for (const certifier of activeRowsfinal) {
      ...
      if (!c) {
        ...
        const qry0 =
          'SELECT "email" FROM "company" WHERE "email" LIKE ' +
          "'%" +
          emailcheck +
          "%'" +
          "";
    ...
    for (const deac_certifiers of deactiveRowsfinal) {
      ...
      if (c) {
        this.logger.error(
          deac_certifiers.entity +
            " This Certifer is withdrawn Deactivate the Account"
        );
      }
    }
  }
