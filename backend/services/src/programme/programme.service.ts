...
export class ProgrammeService {
  ...
  private async doInvestment(
    ...
    const savedProgramme = await this.entityManager
      .transaction(async (em) => {
        ...
        (await this.checkPendingInvestmentValidity(programme, transfer, nationalInvestment)).map(
          async (toRejectId) => {
            await em.update(
              Investment,
              {
    ...
    throw new HttpException(
      this.helperService.formatReqMessagesString("programme.internalErrorStatusUpdating", []),
      HttpStatus.INTERNAL_SERVER_ERROR
    );
  }

  async findPermissionForMinistryUser(user: User, programmeSectoralScope: any): Promise<boolean> {
  ...
  async updateOwnership(
    ...
    if (update.proponentPercentage.reduce((a, b) => a + b, 0) != 100) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.proponentPercSum=100", []),
        HttpStatus.BAD_REQUEST
      );
    }
  ...
  async addInvestment(req: InvestmentRequestDto, requester: User) {
    ...
    if (req.period && (req.period.length != 2 || req.period[0] >= req.period[1])) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.invalidPeriod", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (req.startOfPayback && req.startOfPayback < todaySart) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.paybackStartDate<currentDate", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (req.percentage && req.percentage.reduce((a, b) => a + b, 0) <= 0) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.percentage>0", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    for (const trf of autoApproveInvestmentList) {
      ...
      updateProgramme = (
        await this.doInvestment(
          trf,
          `${toCompany.companyId}#${toCompany.name}#${this.getUserRef(requester)}`
            .split("#", 4)
            .join("#"),
          programme,
          toCompany,
  ...
  private async getCreditRequest(
    ...
    if (
      ...
      sr.energyGenerationUnit = ndcActionDto.solarProperties.energyGenerationUnit;
      if (constants) {
  ...
  private async createNDCActionId(ndcAction: NDCActionDto, programmeId: string) {
    const id = await this.counterService.incrementCount(CounterType.NDC_ACTION, 3);
    ...
    if (
      ...
      if (req) {
        try {
          ...
          ndcAction.constantVersion = constants ? String(constants.version) : "default";
        } catch (e) {
  ...
  async checkTotalUserEstimatedCredits(ndcAction: NDCAction, program: Programme) {
    ...
    ndcActions.forEach((ndcAction: NDCAction) => {
      if (ndcAction.ndcFinancing && ndcAction.ndcFinancing.userEstimatedCredits) {
        totalUserEstimatedCredits += ndcAction.ndcFinancing.userEstimatedCredits;
      }
    });
        ...
        if (program.certifierId) {
          ...
          if (index === -1) {
            await this.programmeLedger.updateCertifier(
              ...
              d.type == DocType.METHODOLOGY_DOCUMENT ? ProgrammeStage.APPROVED : undefined
            );
          }
        } else {
          await this.programmeLedger.updateCertifier(
  ...
  async docAction(documentAction: DocumentAction, user: User) {
    ...
    if (!d) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.documentNotExist", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (d.status == DocumentStatus.ACCEPTED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.documentAlreadyAccepted", []),
        HttpStatus.BAD_REQUEST
      );
    }
    let ndc: NDCAction;
    ...
    return new BasicResponseDto(
      HttpStatus.OK,
      this.helperService.formatReqMessagesString("programme.actionSuccessful", [])
    );
  }
  ...
  async sendLetterOfIntentResponse(programme: Programme) {
    ...
    for (const companyId of programme.companyId) {
      ...
      const letterOfIntentResponseLetterUrl = await this.letterOfIntentResponseGen.generateLetter(
  ...
  async create(programmeDto: ProgrammeDto, user: User): Promise<Programme | undefined> {
    ...
    if (programme.creditEst)
      programme.creditEst = this.helperService.halfUpToPrecision(programme.creditEst);
    ...
    if (
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.govermentOwnershipOfProgramme", []),
        HttpStatus.BAD_REQUEST
      );
    }
    if (
      ...
      (!programmeDto.proponentPercentage ||
        programmeDto.proponentPercentage.length != programmeDto.proponentTaxVatId.length)
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
    ...
    if (
      programmeDto.proponentPercentage &&
      programmeDto.proponentTaxVatId.length != programmeDto.proponentPercentage.length
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "programme.proponentPercAndTaxIdsNotMatched",
    ...
    if (programmeDto.proponentTaxVatId.length !== new Set(programmeDto.proponentTaxVatId).size) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.duplicatedProponentTaxIds", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (
      ...
      if (!sectoralScopesMapped[programmeSector].includes(programmeSectoralScopeKey)) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.wrongSectorAndScopeMapping", []),
          HttpStatus.BAD_REQUEST
        );
      }
    }
    ...
    if (programmeDto.article6trade == false) {
      ...
      if (programmeDto.supportingowners && programmeDto.supportingowners.length > 0) {
    ...
    programme.programmeProperties.estimatedProgrammeCostUSD = this.helperService.halfUpToPrecision(
      programme.programmeProperties.estimatedProgrammeCostUSD
    );
    programme.programmeProperties.carbonPriceUSDPerTon = this.helperService.halfUpToPrecision(
      programme.programmeProperties.estimatedProgrammeCostUSD / programme.creditEst
    );
    ...
    if (
      ...
      if (
        ...
        const certifierId =
          user.companyRole === CompanyRole.CERTIFIER ? Number(user.companyId) : undefined;
  ...
  async addNDCAction(ndcActionDto: NDCActionDto, user: User): Promise<DataResponseDto> {
    ...
    if (ndcAction.action === NDCActionType.Mitigation) {
      if (
        ...
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.wrongMItigationSectorMapping", []),
          HttpStatus.BAD_REQUEST
        );
      }
    ...
    if (
      ...
      const document = (ndcActionDto.coBenefitsProperties as any).assessmentDetails.document;
      if (document) {
        ...
        (ndcActionDto.coBenefitsProperties as any).assessmentDetails.document = response;
      }
    }

    ndcAction.coBenefitsProperties = ndcActionDto.coBenefitsProperties;
    if (ndcAction.ndcFinancing?.userEstimatedCredits) {
      ndcAction.ndcFinancing.userEstimatedCredits = this.helperService.halfUpToPrecision(
        ndcAction.ndcFinancing.userEstimatedCredits
      );
    }
    if (ndcAction.ndcFinancing?.systemEstimatedCredits) {
      ndcAction.ndcFinancing.systemEstimatedCredits = this.helperService.halfUpToPrecision(
        ndcAction.ndcFinancing.systemEstimatedCredits
      );
    }
    if (ndcAction.solarProperties?.energyGeneration) {
      ndcAction.solarProperties.energyGeneration = this.helperService.halfUpToPrecision(
        ndcAction.solarProperties.energyGeneration
      );
    }
    if (ndcAction.creditCalculationProperties?.energyGeneration) {
      ndcAction.creditCalculationProperties.energyGeneration = this.helperService.halfUpToPrecision(
        ndcAction.creditCalculationProperties.energyGeneration
      );
    }
    if (ndcAction.agricultureProperties?.landArea) {
      ndcAction.agricultureProperties.landArea = this.helperService.halfUpToPrecision(
        ndcAction.agricultureProperties.landArea
      );
    }
    if (ndcAction.adaptationProperties?.ghgEmissionsAvoided?.CO2) {
      ndcAction.adaptationProperties.ghgEmissionsAvoided.CO2 = this.helperService.halfUpToPrecision(
        ndcAction.adaptationProperties.ghgEmissionsAvoided.CO2
      );
    }
    if (ndcAction.adaptationProperties?.ghgEmissionsAvoided?.CH4) {
      ndcAction.adaptationProperties.ghgEmissionsAvoided.CH4 = this.helperService.halfUpToPrecision(
        ndcAction.adaptationProperties.ghgEmissionsAvoided.CH4
      );
    }
    if (ndcAction.adaptationProperties?.ghgEmissionsAvoided?.N2O) {
      ndcAction.adaptationProperties.ghgEmissionsAvoided.N2O = this.helperService.halfUpToPrecision(
    ...
    if (ndcAction.adaptationProperties?.ghgEmissionsAvoided?.SF6) {
      ndcAction.adaptationProperties.ghgEmissionsAvoided.SF6 = this.helperService.halfUpToPrecision(
        ndcAction.adaptationProperties.ghgEmissionsAvoided.SF6
      );
    }
    if (ndcAction.adaptationProperties?.ghgEmissionsReduced?.CO2) {
      ndcAction.adaptationProperties.ghgEmissionsReduced.CO2 = this.helperService.halfUpToPrecision(
        ndcAction.adaptationProperties.ghgEmissionsReduced.CO2
      );
    }
    if (ndcAction.adaptationProperties?.ghgEmissionsReduced?.CH4) {
      ndcAction.adaptationProperties.ghgEmissionsReduced.CH4 = this.helperService.halfUpToPrecision(
        ndcAction.adaptationProperties.ghgEmissionsReduced.CH4
      );
    }
    if (ndcAction.adaptationProperties?.ghgEmissionsReduced?.N2O) {
      ndcAction.adaptationProperties.ghgEmissionsReduced.N2O = this.helperService.halfUpToPrecision(
    ...
    if (ndcAction.adaptationProperties?.ghgEmissionsReduced?.SF6) {
      ndcAction.adaptationProperties.ghgEmissionsReduced.SF6 = this.helperService.halfUpToPrecision(
  ...
  async queryNdcDetails(query: QueryDto, abilityCondition: string): Promise<DataListResponseDto> {
    return new DataListResponseDto(undefined, undefined);
  }

  async queryNdcActions(query: QueryDto, abilityCondition: string): Promise<DataListResponseDto> {
    ...
    let queryBuilder = await this.ndcActionViewRepo
      ...
      .where(
        this.helperService.generateWhereSQL(
          query,
          this.helperService.parseMongoQueryToSQLWithTable("ndcaction", abilityCondition),
          "ndcaction"
        )
      );
    ...
    const resp = await queryBuilder
      .orderBy(
        query?.sort?.key && `"ndcaction".${this.helperService.generateSortCol(query?.sort?.key)}`,
        query?.sort?.order,
  ...
  async downloadNdcActions(queryData: DataExportQueryDto, abilityCondition: string) {
    ...
    if (resp.length > 0) {
      ...
      for (const key of titleKeys) {
        headers.push(this.helperService.formatReqMessagesString("ndcActionExport." + key, []));
      }

      const path = await this.dataExportService.generateCsv(
        prepData,
        headers,
        this.helperService.formatReqMessagesString("ndcActionExport.activities", [])
      );
      return path;
    }
    throw new HttpException(
  ...
  async queryDocuments(query: QueryDto, abilityCondition: string): Promise<DataListResponseDto> {
    const skip = query.size * query.page - query.size;
    let resp = await this.documentViewRepo
      ...
      .where(
        this.helperService.generateWhereSQL(
          query,
          this.helperService.parseMongoQueryToSQLWithTable("programmedocument", abilityCondition),
          "programmedocument"
        )
      )
      .orderBy(
  ...
  async transferReject(req: ProgrammeTransferReject, approver: User) {
    ...
    if (!pTransfer) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.transferReqNotExist", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (pTransfer.status == TransferStatus.CANCELLED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.acceptOrRejAlreadyCancelled", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (!pTransfer.isRetirement && pTransfer.fromCompanyId != approver.companyId) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.invalidApproverForTransferReq", []),
        HttpStatus.FORBIDDEN
      );
    }
    if (
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.invalidApproverForRetirementReq", []),
        HttpStatus.FORBIDDEN
      );
    }

    if (approver.companyRole === CompanyRole.MINISTRY) {
      const programme = await this.programmeLedger.getProgrammeById(pTransfer.programmeId);
    ...
    if (result.affected > 0) {
      if (pTransfer.isRetirement) {
        ...
        const omgeCredits =
          pTransfer.retirementType == RetireType.CROSS_BORDER
            ? Number(((pTransfer.creditAmount * pTransfer.omgePercentage) / 100).toFixed(2))
            : 0;
        await this.emailHelperService.sendEmailToOrganisationAdmins(
      ...
      return new BasicResponseDto(
        HttpStatus.OK,
        this.helperService.formatReqMessagesString("programme.transferReqRejectSuccess", [])
      );
    }

    throw new HttpException(
      this.helperService.formatReqMessagesString("programme.noPendReqFound", []),
      HttpStatus.BAD_REQUEST
    );
  }
  ...
  async downloadTransfers(queryData: DataExportQueryDto, abilityCondition: string, user: User) {
    const queryDto = new QueryDto();
    queryDto.filterAnd = queryData.filterAnd;
    queryDto.filterOr = queryData.filterOr;
    queryDto.filterBy = queryData.filterBy;
    ...
    if (resp.length > 0) {
      ...
      const titleKeys = Object.keys(prepData[0]);
      for (const key of titleKeys) {
        headers.push(this.helperService.formatReqMessagesString("transferExport." + key, []));
      }

      const path = await this.dataExportService.generateCsv(
        prepData,
        headers,
        this.helperService.formatReqMessagesString("transferExport.transfers", [])
      );
      return path;
    }
  ...
  private prepareTransferDataForExport(transfers: any) {
    ...
    for (const transfer of transfers) {
      ...
      dto.status =
        transfer.status === TransferStatus.NOTRECOGNISED ? "Not Recognised" : transfer.status;
      ...
      dto.certifier =
        transfer.certifier && Array.isArray(transfer.certifier) && transfer.certifier.length > 0
  ...
  async transferApprove(req: ProgrammeTransferApprove, approver: User) {
    ...
    if (
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.transferAlreadyApproved", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    const giver = await this.companyService.findByCompanyId(transfer.fromCompanyId);
    ...
    if (receiver.state === CompanyState.SUSPENDED) {
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.receiveCompanySuspended", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (giver.state === CompanyState.SUSPENDED) {
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.cerditSendingCompSuspended", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (transfer.status != TransferStatus.PROCESSING) {
      ...
      if (trq.affected <= 0) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.noPendingTransferReq", []),
          HttpStatus.BAD_REQUEST
        );
      }
    }
    ...
    const transferResult = await this.doTransfer(
      transfer,
      `${this.getUserRef(approver)}#${receiver.companyId}#${receiver.name}#${giver.companyId}#${
        giver.name
      }`,
      req.comment,
      transfer.isRetirement
  ...
  async transferCancel(req: ProgrammeTransferCancel, requester: User) {
    ...
    if (transfer.status != TransferStatus.PENDING) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.acceptOrRejCancelledReq", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (result.affected > 0) {
      ...
      return new BasicResponseDto(
        HttpStatus.OK,
        this.helperService.formatReqMessagesString("programme.transferCancelSuccess", [])
      );
    }
    return new BasicResponseDto(
      HttpStatus.BAD_REQUEST,
      this.helperService.formatReqMessagesString("programme.transferReqNotExistinGiv", [])
    );
  }
  ...
  async transferRequest(req: ProgrammeTransferRequest, requester: User) {
    ...
    if (req.companyCredit && req.companyCredit.reduce((a, b) => a + b, 0) <= 0) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.companytotalAmount>0", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (req.fromCompanyIds.length > 1) {
      if (!req.companyCredit) {
        ...
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.invalidCompCreditForGivenComp", []),
          HttpStatus.BAD_REQUEST
        );
      }
    }
    ...
    if (
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.invalidCompCreditFromGivenComp", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (programme.currentStage != ProgrammeStage.AUTHORISED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.programmeNotInCreditIssuedState", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    for (const trf of autoApproveTransferList) {
      ...
      updateProgramme = (
        await this.doTransfer(
          trf,
          `${this.getUserRef(requester)}#${toCompany.companyId}#${toCompany.name}#${
  ...
  async downloadProgrammes(queryData: DataExportQueryDto, abilityCondition: string) {
    ...
    let resp = await this.programmeViewRepo
      ...
      .where(
        this.helperService.generateWhereSQL(
          queryDto,
          this.helperService.parseMongoQueryToSQLWithTable("programme", abilityCondition),
          "programme"
        )
      )
      .leftJoinAndMapMany(
    ...
    if (resp.length > 0) {
      ...
      const titleKeys = Object.keys(prepData[0]);
      for (const key of titleKeys) {
        headers.push(this.helperService.formatReqMessagesString("programmeExport." + key, []));
      }

      const path = await this.dataExportService.generateCsv(
        prepData,
        headers,
        this.helperService.formatReqMessagesString("programmeExport.projects", [])
      );
      return path;
    }
  ...
  async query(query: QueryDto, abilityCondition: string): Promise<DataListResponseDto> {
    ...
    let resp = await this.programmeViewRepo
      ...
      .orderBy(
        query?.sort?.key && `"programme".${this.helperService.generateSortCol(query?.sort?.key)}`,
        query?.sort?.order,
        query?.sort?.nullFirst !== undefined
          ? query?.sort?.nullFirst === true
            ? "NULLS FIRST"
  ...
  async certify(req: ProgrammeCertify, add: boolean, user: User) {
    ...
    if (user.companyRole === CompanyRole.GOVERNMENT || user.companyRole === CompanyRole.MINISTRY) {
      if (!req.certifierId) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.certifierIdRequiredForGov", []),
          HttpStatus.FORBIDDEN
        );
      }
    ...
    if (userCompany && userCompany.state === CompanyState.SUSPENDED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.organisationDeactivated", []),
        HttpStatus.FORBIDDEN
      );
    }
    ...
    if (add) {
      return new DataResponseMessageDto(
        HttpStatus.OK,
        this.helperService.formatReqMessagesString("programme.certifyPendingProgramme", []),
        updated
      );
    } else {
      return new DataResponseMessageDto(
        HttpStatus.OK,
        this.helperService.formatReqMessagesString("programme.certificationRevocation", []),
        updated
      );
    }
  }
  ...
  async retireProgramme(req: ProgrammeRetire, requester: User) {
    ...
    if (req.type == RetireType.CROSS_BORDER) {
      ...
      if (req.omgePercentage != govProfile.omgePercentage) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.invalidOmgePerc", []),
          HttpStatus.BAD_REQUEST
        );
      }
    }
    ...
    if (
      ...
      if (!req.fromCompanyIds.includes(requester.companyId)) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.reqNotIncludedInFromCompanyId", []),
          HttpStatus.BAD_REQUEST
        );
      }
      ...
      if (!req.companyCredit) {
        ...
        req.companyCredit = [
          this.helperService.halfUpToPrecision(
            this.helperService.halfUpToPrecision(
              (programme.creditBalance * programme.creditOwnerPercentage[reqIndex]) / 100
      ...
      if (
        ...
        await this.emailHelperService.sendEmailToOrganisationAdmins(
          ...
          EmailTemplates.CREDIT_RETIREMENT_BY_GOV,
          {
            ...
            omgeRetireDesc:
              req.type == RetireType.CROSS_BORDER
                ? this.helperService.formatReqMessagesString("programme.omgeRetireDesc", [
                    omgeCredits,
                  ])
                : "",
          }
  ...
  async issueProgrammeCredit(req: ProgrammeMitigationIssue, user: User) {
    ...
    if (program.currentStage != ProgrammeStage.AUTHORISED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.notInAUthorizedState", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    req.issueAmount.map((action) => {
      if (countedActions.includes(action.actionId)) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.duplicateMitigationActionIds", [
            action.actionId,
          ]),
          HttpStatus.BAD_REQUEST
        );
      ...
      verfiedMitigationMap[action.actionId].availableCredits = this.helperService.halfUpToPrecision(
        verfiedMitigationMap[action.actionId].availableCredits - action.issueCredit
      );
      verfiedMitigationMap[action.actionId].issuedCredits = this.helperService.halfUpToPrecision(
    ...
    let updated: any = await this.programmeLedger.issueProgrammeStatus(
      ...
      program.companyId,
      totalCreditIssuance,
      `${this.getUserRefWithRemarks(user, req.comment)}#${this.getNdcCreditIssuanceRef(
        req.issueAmount
      )}`,
      program.mitigationActions
    );
    if (!updated) {
      return new BasicResponseDto(
        HttpStatus.BAD_REQUEST,
        this.helperService.formatReqMessagesString("programme.notFOundAPendingProgrammeForTheId", [
          req.programmeId,
        ])
      );
    }
  ...
  async issueCredit(issue: ProgrammeMitigationIssue, abilityCondition: string) {
    ...
    if (parseFloat(String(programme.creditIssued)) + totalCreditIssuance > programme.creditEst) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
  ...
  async authorizeProgramme(req: ProgrammeApprove, user: User, auth_letter?: string) {
    ...
    if (program.article6trade == false) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.programmeCannotbeAuthorized", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (program.currentStage != ProgrammeStage.APPROVED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.notInPendingState", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (!updated) {
      return new BasicResponseDto(
        HttpStatus.BAD_REQUEST,
        this.helperService.formatReqMessagesString("programme.inotFOundAPendingProgrammeForTheId", [
          req.programmeId,
  ...
  async rejectProgramme(req: ProgrammeReject, user: User) {
    if (
      ...
      if (currentStage === ProgrammeStage.REJECTED) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.rejectAlreadyRejectedProg", []),
          HttpStatus.BAD_REQUEST
        );
      }
      ...
      if (programme.article6trade == false) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.programmeCannotbeReject", []),
          HttpStatus.BAD_REQUEST
        );
      }
  ...
  async regenerateRegionCoordinates() {
    ...
    for (const programme of allProgrammes) {
      ...
      let address: any[] = [];
      if (programmeProperties.geographicalLocation) {
        for (let index = 0; index < programmeProperties.geographicalLocation.length; index++) {
          address.push(programmeProperties.geographicalLocation[index]);
        }
      }
  ...
  async investmentCancel(req: InvestmentCancel, requester: User) {
    this.logger.log(
      `Investment cancel by ${requester.companyId}-${requester.id} received ${JSON.stringify(req)}`
    );
    ...
    if (!investment) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.investmentReqDoesNotExist", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (investment.initiatorCompanyId != requester.companyId) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.invalidApproverForInvestmentReq", []),
        HttpStatus.FORBIDDEN
      );
    }
    ...
    if (result.affected > 0) {
      return new BasicResponseDto(
        HttpStatus.OK,
        this.helperService.formatReqMessagesString("programme.investmentCancelSuccess", [])
      );
    }
    return new BasicResponseDto(
      HttpStatus.BAD_REQUEST,
      this.helperService.formatReqMessagesString("programme.investmentReqNotExistinGiv", [])
    );
  }

  async investmentReject(req: InvestmentReject, approver: User) {
    this.logger.log(`Investment reject ${JSON.stringify(req)} ${approver.companyId}`);
    ...
    if (result.affected > 0) {
      return new BasicResponseDto(
        HttpStatus.OK,
        this.helperService.formatReqMessagesString("programme.investmentReqRejectSuccess", [])
      );
    }
  ...
  async investmentApprove(req: InvestmentApprove, approver: User) {
    ...
    if (investment.status == InvestmentStatus.APPROVED) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.investmentAlreadyApproved", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (investment.nationalInvestmentId && !nationalInvestment) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.nationalInvestmentDoesNotExist", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    const receiver = await this.companyService.findByCompanyId(investment.toCompanyId);
    const giver = await this.companyService.findByCompanyId(investment.fromCompanyId);
  ...
  async addNdcDetailsPeriod(
    ...
    await this.ndcDetailsPeriodRepo.save(addedNdcDetailsPeriod).catch((error) => {
      ...
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.ndcActionPeriodCreateFailed", []),
        HttpStatus.BAD_REQUEST
      );
    });
  ...
  async addNdcDetailAction(
    ...
    await this.ndcDetailsActionRepo.save(addedNdcDetailsAction).catch((error) => {
      this.logger.error(error);
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.ndcActionCreateFailed", []),
        HttpStatus.BAD_REQUEST
      );
    });
  ...
  async updateNdcDetailsAction(
    ...
    if (!ndcAction) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.ndcActionNotExist", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (!company) {
      throw new HttpException(
        this.helperService.formatReqMessagesString("programme.companyNotExist", []),
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (ndcDetailsAction.actionType === NdcDetailsActionType.SubAction) {
      if (
        !(
          user.companyRole === CompanyRole.GOVERNMENT ||
          (user.companyRole === CompanyRole.MINISTRY && company.name === ndcAction.ministryName)
        )
      ) {
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.unAuth", []),
    ...
    const result = await this.ndcDetailsActionRepo
      ...
      .catch((error) => {
        this.logger.error(error);
        throw new HttpException(
          this.helperService.formatReqMessagesString("programme.ndcActionUpdateFailed", []),
          HttpStatus.BAD_REQUEST
        );
      });
