...
- **PD and IC can read CorrespondingAdjustment records via API (CASL leak).** `casl-ability.factory.ts` grants `Action.Read` on `CorrespondingAdjustment` to every authenticated role (DNA/Ministry/PD/IC). `POST /national/correspondingAdjustment/query` returns 200 with the full list for any authenticated user, even though the UI sider hides the menu item for non-DNA users. The UI is the only gate. Flagged as `test.fixme` in `tests/e2e/article6/corresponding-adjustment.spec.ts:527` pending a CASL-factory tightening.
