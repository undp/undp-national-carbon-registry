...
- **CRITICAL: `hasSubmittedReport` is never called as a guard before ITMO authorization.** A project-wide grep for `hasSubmittedReport` (backend/ on 2026-04-19) returns exactly three hits — the service method declaration, the controller's `/check` handler, and the `check` endpoint's response field. **No ITMO authorization / issuance / first-transfer code path consults it.** This means the registry today does not enforce Decision 2/CMA.3 para 18 — a DNA Admin can issue the first ITMO for a cooperative approach without ever having generated an initial report, let alone having submitted one. The `/check` endpoint exists as a read-only probe for downstream integration, but no integration has been wired. A future phase that wants to honor para 18 must either (a) add a `hasSubmittedReport` check to the authorization service's pre-conditions, or (b) add a DB-level constraint linking credit-authorization rows to a submitted IR.
