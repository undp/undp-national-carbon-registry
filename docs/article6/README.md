...
3. **Most ITMO lifecycle events are not HTTP-exposed** — `retireToAccount` remains service-internal. ~~`ItmoAccount` CRUD~~ — **RESOLVED**: `POST /national/itmoAccount/query` + `GET /national/itmoAccount/byCompany` now expose the Dec 2/CMA.3 Annex para 29 per-account holdings (DNA read-only via CASL). ~~the four new Article 6.2 retirement types (`USE_TOWARDS_NDC`, `USE_FOR_OIMP`, `OMGE_CANCELLATION`, `SOP_ADAPTATION`) are service-internal; the UI retirement modal only exposes the two pre-Article-6.2 options.~~ **RESOLVED for retirement types** — the UI retirement modal now exposes all 6 radios, and `ProgrammeLedgerService.addRetireActionResponse` dispatches via a new `mapRetirementTypeToAccountType` helper that sets `accountType` on the derived retirement block so AEF Actions rows reflect the authorizing Party's intent. **First-transfer toggling is RESOLVED** (see entry 3b below). [02-itmo-lifecycle.md](./02-itmo-lifecycle.md)
