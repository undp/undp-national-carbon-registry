...
export class ProgrammeService {
  ...
  async transferReject(
    req: ProgrammeTransferReject,
    approverCompanyId: number
  ) {
    this.logger.log(
      `Programme reject ${JSON.stringify(req)} ${approverCompanyId}`
    );
  ...
  async transferApprove(
    ...
    const received = await this.companyService.findByCompanyId(
    ...
    return await this.doTransfer(
      transfer,
      `${received.companyId}#${received.name}#${user.id}#${user.name}`,
      req.comment,
  ...
  async transferRequest(req: ProgrammeTransferRequest, requester: User) {
    ...
    for (const j in req.fromCompanyIds) {
      ...
      if (!programme.companyId.includes(fromCompanyId)) {
        throw new HttpException(
          "Transfer request from company does own the programme",
          HttpStatus.BAD_REQUEST
        );
      }
  ...
  async retireProgramme(req: ProgrammeRetire, requester: User) {
    ...
    if (
      ...
      !programme.companyId.includes(requester.companyId)
    ) {
      throw new HttpException(
  ...
  async issueProgrammeCredit(req: ProgrammeIssue, user: User) {
    ...
    if (program.currentStage != ProgrammeStage.ISSUED) {
      throw new HttpException(
        "Programme is not in issued state",
        HttpStatus.BAD_REQUEST
      );
    }
